...
/*
 * Här låg tidigare getDailyStats och getTeacherStats: en andra, lokal
 * uppsättning lärarstatistik som räknade texter, genrer, teman och en
 * topplista ur localStorage. Ingenting renderade den. Lärarvyn hämtar sin
 * statistik från servern via /api/stats/get, och den lokala varianten kunde
 * bara ge siffror för den enskilda enheten – aldrig för en hel klass.
 *
 * Två system som ser ut att göra samma sak är en fälla: nästa gång någon
 * rättar ett fel i lärarstatistiken finns risken att rättningen hamnar i det
 * som ingen visar. Därför är den borttagen. Se api/stats/get.ts.
 *
 * recordDailyStats ovan är kvar – den anropas när en text blir klar.
 */
