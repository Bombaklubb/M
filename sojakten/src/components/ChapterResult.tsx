...
export default function ChapterResult() {
  const { lastResult, selectedChapter, selectedSubject, selectedGrade, setView, selectChapter, openChapterStudy, startExitTicket } = useApp();

  useEffect(() => {
    if (!lastResult || !selectedChapter || !selectedSubject) setView('subject-select');
  }, [lastResult, selectedChapter, selectedSubject, setView]);

  if (!lastResult || !selectedChapter || !selectedSubject) return null;
  ...
  const grade = Number(selectedChapter.grade ?? selectedGrade ?? 5);
