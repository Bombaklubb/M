...
export default function Veckoplaneraren() {
  const [temaIds, setTemaIds] = useState<string[]>([ALLA_TEMAN[0].id])
  ...
  const valdaTeman = useMemo(
    () => ALLA_TEMAN.filter((t) => temaIds.includes(t.id)),
    [temaIds],
  )
