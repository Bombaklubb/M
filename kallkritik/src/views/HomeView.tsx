...
import { Zap, Flame, Trophy, ChevronDown, ChevronUp, Lightbulb, GraduationCap, Star } from 'lucide-react';
      ...
      >
        <div className="relative rounded-[24px] overflow-hidden border-[3px] border-indigo-200 bg-gradient-to-br from-indigo-500 via-violet-500 to-purple-600 px-6 py-8 shadow-[0_8px_0_0_rgba(99,102,241,0.35)]">
          ...
          <div className="relative">
            ...
            >
              {[
                { icon: <Star className="w-3.5 h-3.5" />, label: '6 Moduler' },
        ...
        <AnimatePresence>
          {allCompleted && (
            <motion.div
              ...
              className="mt-4 bg-gradient-to-r from-amber-400 to-orange-400 border-[3px] border-amber-500 rounded-[20px] px-5 py-4 flex items-center gap-4 shadow-[0_6px_0_0_rgba(245,158,11,0.4)]"
            >
              ...
              <Star className="w-6 h-6 text-white ml-auto opacity-80" />
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
      ...
      >
        ...
        >
          ...
          <div className="flex-1">
            ...
            <div className="text-xs font-semibold text-gray-400">Klassrumstips och pedagogiska idéer</div>
          </div>
          {tipsExpanded
            ...
            >
              <div className="px-5 pb-5 space-y-4 border-t-2 border-dashed border-indigo-100">
                <p className="text-sm text-gray-500 leading-relaxed pt-4 font-medium">
                  ...
                  <span className="text-amber-600 font-bold">diskussionsfrågor</span> som dyker upp efter varje svar.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    { icon: '👥', title: 'Gruppspel', desc: 'Spela en modul gemensamt på storskärm och diskutera svaren.', color: 'bg-violet-50 border-violet-200' },
                    { icon: '🏆', title: 'Tävling', desc: 'Låt eleverna tävla om vem som får flest XP i en session.', color: 'bg-amber-50 border-amber-200' },
                    { icon: '💬', title: 'Diskussion', desc: 'Använd diskussionsprompts i Modul 6 som startpunkt för samtal.', color: 'bg-cyan-50 border-cyan-200' },
                    { icon: '📝', title: 'Hemuppgift', desc: 'Eleverna spelar en modul hemma och återberättar vad de lärde sig.', color: 'bg-emerald-50 border-emerald-200' },
                  ].map(tip => (
