...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const [visitCounts, exerciseCounts, totalTimeSec, errorKeys, heartbeatKeys] = await Promise.all([
      ...
      Promise.all(days30.map(d => redis.pfcount(`visits:daily:${d}`))),
      // Exercise counts per day
      Promise.all(days30.map(d => redis.get<number>(`exercises:daily:${d}`))),
      // Total time in seconds
      redis.get<number>('time:total'),
      // All error topic keys
      redis.keys('errors:topic:*'),
    ...
    const visitorsMonth = await redis.pfcount(...days30.map(d => `visits:daily:${d}`));
    ...
    if (errorKeys.length > 0) {
      ...
      topErrors = errorKeys
        .map((k, i) => ({
          topic: String(k).replace('errors:topic:', ''),
    ...
    const totalTimeHours = Math.round((totalTimeSec ?? 0) / 60); // → minutes
    ...
    res.status(200).json({
      ...
      totalExercises,
      totalErrors,
      totalTimeMinutes: totalTimeHours,
      topErrors,
      daily14,
    });
  } catch (err) {
