...
const QUESTION_TYPE_LABELS: Record<QuestionType, { label: string; emoji: string }> = {
  literal: { label: 'On the Lines', emoji: '🔍' },
  inference: { label: 'Between the Lines', emoji: '🧠' },
};
...
export const ProfileView: React.FC<ProfileViewProps> = ({ user, onClose, onAvatarChange }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-4">
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl border-b-8 border-indigo-100 dark:border-indigo-900 p-5">
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-extrabold text-indigo-900 dark:text-indigo-300">Min profil</h1>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          ...
          <div className="bg-indigo-50 dark:bg-indigo-900/30 border-2 border-indigo-200 dark:border-indigo-700 rounded-xl p-3 text-center">
            <span className="text-2xl">📖</span>
            <div className="text-xl font-bold text-indigo-700 dark:text-indigo-300">{user.completedTexts.length}</div>
            <div className="text-xs text-indigo-600 dark:text-indigo-400">Texter</div>
          </div>
          <div className="bg-purple-50 dark:bg-purple-900/30 border-2 border-purple-200 dark:border-purple-700 rounded-xl p-3 text-center">
            <span className="text-2xl">🏅</span>
            <div className="text-xl font-bold text-purple-700 dark:text-purple-300">{user.badges.length}</div>
            <div className="text-xs text-purple-600 dark:text-purple-400">Achievements</div>
          </div>
        </div>
        ...
        {user.completedTexts.length > 0 && (
          <div className="mb-4">
            <h3 className="text-base font-bold text-slate-700 dark:text-slate-200 mb-2">Statistics</h3>
        ...
        {/* Badges */}
        <div>
          <h3 className="text-base font-bold text-slate-700 dark:text-slate-200 mb-2">Achievements</h3>
        ...
        <button
          onClick={onClose}
          className="w-full mt-4 py-2 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition"
        >
          Close
        </button>
      </div>
